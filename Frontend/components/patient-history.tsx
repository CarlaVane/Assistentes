  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const patientDataResponse = await getPacienteDetailsRequest(token, pIdProp);
        let pacienteIdFromDetails: string | undefined = undefined;
        if (patientDataResponse.ok) {
          ...
          const normalized: PatientData = {
            ...
            weight:
              typeof raw?.weight === "number"
                ? `${raw.weight} kg`
                : typeof raw?.peso === "number"
                ? `${raw.peso} kg`
                : String(raw?.weight ?? raw?.peso ?? ""),
            height:
              typeof raw?.height === "number"
                ? `${raw.height} m`
                : typeof raw?.altura === "number"
                ? `${raw.altura} m`
                : String(raw?.height ?? raw?.altura ?? ""),
            chronicDiseases:
              Array.isArray(raw?.chronicDiseases)
                ? raw.chronicDiseases
                : Array.isArray(raw?.predisposicoes)
                ? raw.predisposicoes
                : [],
          ...
          try {
            const userRes = await getUserRequest(user.id, token);
            if (userRes.ok) {
              const payload = await userRes.json();
              const u = payload?.data ?? payload;
              const normalizedFromUser: PatientData = {
                name: String(u?.nome ?? ""),
                documento: "",
              ...
              const errBody = await userRes.json().catch(() => ({} as any));
