...
function PatientCard({
  ...
  return (
    <Card
      className={`cursor-pointer hover:shadow-lg hover:scale-[1.02] transition-transform duration-200 ${consultation.status === "preliminar"
...
// --------------------- DASHBOARD ---------------------
export function DoctorDashboard({ preloadedConsultations }: { preloadedConsultations?: any } = {}) {
  const router = useRouter();
  const [searchQuery, setSearchQuery] = useState("");
  const [consultations, setConsultations] = useState<Consultation[]>(preloadedConsultations || []);
