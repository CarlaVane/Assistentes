...
function validateConsultaOwnershipAdvanced(req, res, next) {
    return async (req, res, next) => {
        try {
            ...
            // Permitir médicos atualizarem consultas preliminares mesmo sem estarem designados
            const isMedicoUpdatingPreliminar = req.user.tipo === 'medico' && consulta.status === 'preliminar';

            if (!isPacienteOwner && !isMedicoAssigned && !isMedicoUpdatingPreliminar) {
                return res.status(403).json({
                    data: null,
